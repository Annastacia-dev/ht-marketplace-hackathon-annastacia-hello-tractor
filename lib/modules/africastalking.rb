  ...
  class Africastalking
    def initialize
      if Rails.env.production?
        username = Rails.application.credentials.africas_talking[:prod_username] #ENV.fetch('AT_PROD_USERNAME')
        api_key = Rails.application.credentials.africas_talking[:prod_api_key] #ENV.fetch('AT_PROD_API_KEY')
      else
